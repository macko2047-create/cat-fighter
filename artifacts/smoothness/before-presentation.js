...
((root)=>{
  ...
  function create(){
    ...
    let frameKind='frozen',frameCorrection=0,appliedCorrection=0;
    ...
    return {accept,reset,
      render(state,now,input,ready){const previous=lastFrame;frameKind='frozen';frameCorrection=appliedCorrection=0;const out=render(state,now,input,ready);record('frames',{interval:previous?now-previous:null,kind:frameKind,predicted:!!predicted,corrected:appliedCorrection>.01,snapped:frameCorrection>80,correction:frameCorrection,appliedCorrection,buffered:history.filter(s=>s.time>now-100).length,stale:!!history.length&&now-history.at(-1).time>250});return out;},
      resetDiagnostics(){for(const key in samples)samples[key]=[];},
      diagnostics(){const frames=samples.frames,total=frames.length;return {scope:'last 1800 samples per series; milliseconds and game pixels',arrivalMs:stats(samples.arrivals.map(s=>s.interval).filter(x=>x!==null)),simulationMs:stats(samples.arrivals.map(s=>s.simulationMs).filter(x=>x!==null)),frameMs:stats(frames.map(s=>s.interval).filter(x=>x!==null)),renderDelayMs:100,frames:total,categories:Object.fromEntries(['interpolated','frozen','predicted','corrected','snapped','stale'].map(k=>{const count=frames.filter(f=>f.kind===k||f[k]===true).length;return [k,{count,percent:total?100*count/total:0}];})),arrivalCorrectionPx:stats(samples.corrections),appliedCorrectionPx:stats(frames.map(f=>f.appliedCorrection)),reconciliationErrorPx:stats(frames.map(f=>f.correction)),bufferedSnapshots:stats(frames.map(f=>f.buffered))};}
    };
  }
