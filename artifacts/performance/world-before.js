...
function drawWorld(ctx, W, H, t) {
  ...
  // Subtle moving glints over the painted water; center stays legible.
  ctx.save();
  for (let i = 0; i < 22; i++) {
    const x = W * .3 + ((i * 137) % (W * .4));
    const y = (i * 89 + t * 22) % H;
    ctx.globalAlpha = .12 + .1 * Math.sin(t * 1.8 + i);
    ctx.fillStyle = "#d7f5e7"; ctx.fillRect(x, y, 3, 1);
  }
  ctx.restore();
}
