...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<2;i++){const context=await browser.newContext();const p=await context.newPage();p.on('pageerror',e=>errors.push(e.message));await p.goto(base+'?transport=lan');await p.evaluate(()=>{document.documentElement.requestFullscreen=()=>Promise.resolve();document.querySelector('#boot').onclick();arcade.frame(3);});pages.push(p);}
