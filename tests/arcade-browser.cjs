...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>arcade.frame(150));
    assert.equal(await page.evaluate(()=>arcade.phase),'game','normal browser GAME OVER remains until the player chooses');
    const kiosk=await browser.newPage({viewport:{width:390,height:844}});
    await kiosk.addInitScript(()=>{window.requestAnimationFrame=()=>0;});
    await kiosk.goto('file://'+path.resolve('index.html')+'?arcade=1');
    await kiosk.evaluate(()=>{document.documentElement.requestFullscreen=()=>Promise.resolve();$('#boot').click();arcade.frame(3);$('#demo-start').click();start();mode='over';show('GAME OVER','Test');arcade.frame(15);});
    assert.equal(await kiosk.evaluate(()=>arcade.phase),'demo','explicit arcade mode keeps attract auto-return');
    await kiosk.close();
    assert.deepEqual(errors,[]);
    console.log('PASS: BOOT, real demo kills/pickups/bombs/Boss damage, chapters and wrap, state isolation and exception recovery, visible gestures, fixed-player START, dedicated retry, persistent normal Game Over and arcade-only auto-return.');
