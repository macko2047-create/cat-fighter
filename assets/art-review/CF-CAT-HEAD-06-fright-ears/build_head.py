...
def sphere(name,loc,scale,material=None):
    ...
    smooth(o); return o
head=sphere('Head • fused cheeks and ears',(0,0,0),(.67,.50,.48),orange)
...
for v in head.data.vertices:
    ...
    # The finished reference shows a deep cranium and a distinct forward cheek
    # mass. Preserve the rounded back while projecting the lower face slightly.
    v.co.y *= 1.06 if y < 0 else .94
    if y < 0:
        cheek=math.exp(-((z+.12)/.24)**2)*math.exp(-(x/.48)**4)
        v.co.y -= .038*cheek
...
for v in hurt.data:
    ...
    # Start above the submerged root. The upper ear rotates almost as one solid
    # cartilage plate; only the root transition blends, preventing a collapsed
    # dent in the crown.
    root_weight=smoothstep(.28,.42,z)*smoothstep(.34,.45,abs(x))
    pivot=Vector((side*.385,-.045,.18))
    source=Vector((side*.105,0,.56)).normalized()
    target=Vector((side*.50,.34,.055)).normalized()
    rotation=source.rotation_difference(target)
    rotated=pivot+rotation@(Vector((x,y,z))-pivot)
    v.co=Vector((x,y,z)).lerp(rotated,root_weight)
nose=sphere('Nose',(0,-.552,-.075),(.045,.020,.028),pink)
...
for side in [-1,1]:
    normal_eyes.append(sphere('Open eye '+str(side),(side*.245,-.531,.055),(.043,.016,.065),dark))
    pts=[(side*.29,-.542,.070),(side*.24,-.549,.045),(side*.285,-.543,.020)]
    hurt_eyes.append(curve('Squeezed eye '+str(side),pts,.009,dark))
    normal_eyes.append(curve('Smile '+str(side),[(0,-.558,-.102),(side*.04,-.560,-.16),(side*.095,-.555,-.135)],.008,dark))
    hurt_eyes.append(curve('Pained mouth '+str(side),[(0,-.558,-.102),(side*.035,-.560,-.15),(side*.075,-.555,-.18)],.007,dark))
    drop=sphere('Sweat drop '+str(side),(side*.425,-.465,.125),(.025,.018,.064),blue)
    ...
    for row,dz in enumerate([.045,-.025,-.095]):
        curve('Whisker '+str(side)+' '+str(row),[(side*.25,-.545,-.06+dz),(side*.43,-.495,-.035+dz),(side*.60,-.40,-.025+dz)],.005,cream)
