...
func make_p1() -> Node3D:
	...
	# Longitudinal axis: Z. The nose points to -Z (screen-up); pitch only rotates this root around lateral X.
...
func render_pose(viewport: SubViewport, model: Node3D, filename: String, pitch_degrees: float) -> void:
	# Lateral wing-to-wing X-axis only. Yaw (Y) and Roll (Z) deliberately remain zero.
	model.rotation = Vector3(deg_to_rad(pitch_degrees), 0, 0)
	await process_frame
	await process_frame
...
func build_review_sheet() -> void:
	...
	for index in range(3):
		var filename: String = ["p1-pitch-up.png", "p1-neutral.png", "p1-pitch-down.png"][index]
		var pose := Image.load_from_file(OUTPUT + filename)
		review.blend_rect(pose, Rect2i(Vector2i.ZERO, SIZE), Vector2i(index * 512, 0))
	review.save_png(OUTPUT + "p1-pitch-review.png")
...
func _build_and_render() -> void:
	...
	await render_pose(viewport, model, "p1-pitch-up.png", 10.0)
	await render_pose(viewport, model, "p1-neutral.png", 0.0)
	await render_pose(viewport, model, "p1-pitch-down.png", -10.0)
	build_review_sheet()
	quit()
