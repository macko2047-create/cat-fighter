...
func attach_imported_cat_head() -> void:
	# Text3D's FBX is a single, untextured mesh.  Its own Y-up coordinate system
	# is retained; this transform seats it on P1's jacket and matches the former
	# pilot silhouette without changing the aircraft's pivot or flight axes.
	var head := preload("res://models/p1-cat-head.fbx").instantiate() as Node3D
	head.name = "P1ImportedCatHead"
	head.position = Vector3(0,0.54,-0.52)
	head.rotation_degrees = Vector3(0,180,0)
	head.scale = Vector3(0.33,0.82,0.70)
	_apply_head_material(head)
	add_child(head)

func _apply_head_material(node: Node) -> void:
	if node is MeshInstance3D:
		node.material_override = orange
	for child in node.get_children():
		_apply_head_material(child)
